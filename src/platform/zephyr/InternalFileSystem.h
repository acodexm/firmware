...
// InternalFileSystem.h - board-independent Zephyr LittleFS backend
//
// Implements the Adafruit InternalFileSystem API subset used by Meshtastic,
// backed by Zephyr's fs/littlefs on the DTS storage_partition. Partition size
// is taken from DTS at compile time via FIXED_PARTITION_SIZE.
//
// Mount point: /lfs
...
//
// File objects are copyable via std::shared_ptr<ZephyrFileState>.
...
struct ZephyrFileState {
    ...
    char relpath[ZEPHYR_FS_PATHLEN] = {0};
...
class File
{
  public:
    ...
    explicit File(std::shared_ptr<ZephyrFileState> s) : _s(std::move(s)) {}

  private:
    std::shared_ptr<ZephyrFileState> _s;
};
...
class InternalFileSystem
{
  public:
    ...
    uint32_t usedBytes()
    {
        ...
        if (fs_statvfs(ZEPHYR_FS_MOUNT, &st) != 0)
            return 0;
