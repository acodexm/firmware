 ...
 *
 * Arduino.h - Arduino compatibility surface for Zephyr
 *
 * Provides the bounded Arduino API subset retained Meshtastic code uses.
 * Timing, GPIO, interrupt and serial output have concrete Zephyr backends.
 */
...
// A function-like abs macro rewrites std::abs inside libstdc++ headers.
using std::abs;
...
// nRF52 pin numbering: P0.n = n, P1.n = 32+n. nRF54 keeps its 16-pin ports.
...
class String
{
  public:
    ...
    {
        char tmp[32];
        snprintf(tmp, 32, "%.*f", d, static_cast<double>(n));
        assign(tmp, strlen(tmp));
    }
    // cppcheck-suppress noExplicitConstructor
...
// ── strlcpy - BSD extension; implementation in zephyr_arduino.cpp ──────────
#ifndef HAVE_STRLCPY
#define HAVE_STRLCPY
#ifdef __cplusplus
extern "C" {
...
// Used by memGet.cpp when ARCH_NRF52 is defined.
